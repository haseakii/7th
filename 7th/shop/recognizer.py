...
# 有效物品类型 & 货币类型
# ---------------------------------------------------------------------------
VALID_ITEM_TYPES = ("bookmark", "mystic_medal", "equipment", "fodder", "unknown")
...
# 匹配名称区域中出现的文本 → 物品类型
# ---------------------------------------------------------------------------
ITEM_NAME_KEYWORDS: Dict[str, Tuple[str, float]] = {
    # 书签 —  covenant bookmark
    "书签": ("bookmark", 0.95),
    # 神秘奖牌 / 徽章 — mystic medal
...
# 注意：不包含"英雄" — 英雄标记既用于装备也用于角色单位，易误判
...
class ItemRecognizer:
    ...
    def _classify_by_text(combined: str) -> Tuple[str, float]:
        """通过文本匹配关键词确定物品类型（纯文本，无需 OCR）。"""
        if not combined.strip():
            return "unknown", 0.0

        matched_type = "unknown"
        best_conf = 0.0
        for keyword, (item_type, conf) in ITEM_NAME_KEYWORDS.items():
    ...
    def _parse_row_blocks(self, blocks, y1: int, y2: int, slot_index: int) -> ShopItem:
        ...
        # 物品分类（纯文本匹配）
        combined_name = "".join(name_texts)
        item_type, confidence = self._classify_by_text(combined_name)
    ...
    def _recognize_shelf_area(self, image: np.ndarray) -> List[ShopItem]:
        ...
        for slot_index, (y1, y2) in enumerate(item_rows):
            row_blocks = [b for b in all_blocks if y1 <= b.cy <= y2]
