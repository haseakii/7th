...
# 确保 stdout 使用 UTF-8 编码，避免中文/Unicode 字符在控制台乱码
try:
...
if getattr(sys, 'frozen', False):
    ...
    _BASE_DIR = Path(__file__).resolve().parent.parent  # module/ 的上一级 = 项目根目录
_SCREENSHOT_DIR = _BASE_DIR / "screenshots" / "error"


def save_error_screenshot(image, name: str = "") -> str:
    """
    ...
    try:
        ...
        logger.warning(f"无法保存截图: cv2 或 numpy 未安装")
        return ""
