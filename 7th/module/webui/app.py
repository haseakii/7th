...
class AlasGUI(Frame):
    ...
    def dev_utils(self) -> None:
        ...
        def _force_restart():
            if State.restart_event is not None:
                toast("E7 will restart in 3 seconds", duration=0, color="error")
                clearup()
        ...
        # ── 截图性能测试 ─────────────────────────────────────────────
        def _benchmark_screenshot():
            serial = self.alas_config.serial if hasattr(self, 'alas_config') and self.alas_config else "127.0.0.1:16384"
            clear("benchmark_results")
            with use_scope("benchmark_results"):
                put_text("正在测试所有截图方式，请稍候...").style("color: var(--bs-info)")
                from module.device.screenshot import benchmark_all
                import time as _time
                _time.sleep(0.1)
                results = benchmark_all(serial, iterations=3)

                put_text("").style("margin-top: .5rem")
                put_text("截图性能测试结果").style("font-size: 1.1rem; font-weight: bold")

                table_data = [["策略", "平均耗时", "状态"]]
                best_avg = float("inf")
                for r in results:
                    if r["success"] and r["avg_ms"] is not None and r["avg_ms"] < best_avg:
                        best_avg = r["avg_ms"]
                    status = f"{r['avg_ms']:.1f}ms" if r["success"] else f"不可用"
                    if r["note"]:
                        status += f" ({r['note']})"
                    table_data.append([r["name"], status, "✅" if r["success"] else "❌"])
                ...
                if working:
                    best = working[0]
                    put_text(
                        f"推荐: {best['name']} ({best['avg_ms']:.1f}ms)"
                    ).style("color: var(--bs-success); font-weight: bold; margin-top: .5rem")

                    # ── 自动保存最优方案到配置 ───────────────────────────
                    try:
                        from module.config.utils import filepath_config, read_file, write_file
                        config_name = self.alas_name or "default"
                        config_path = filepath_config(config_name)
                        config_data = read_file(config_path)
                        # 更新 ScreenshotMethod
                        if "Device" not in config_data:
                            config_data["Device"] = {}
                        if "Connection" not in config_data["Device"]:
                            config_data["Device"]["Connection"] = {}
                        config_data["Device"]["Connection"]["ScreenshotMethod"] = best["name"]
                        write_file(config_path, config_data)
                        put_text(
                            f"已自动设置截图方式为: {best['name']}"
                        ).style("color: var(--bs-info); margin-top: .25rem")
                    except Exception as e:
                        logger.warning(f"自动保存截图配置失败: {e}")
                        put_text(
                            f"自动保存失败: {e}"
                        ).style("color: var(--bs-warning); font-size: 0.9rem; margin-top: .25rem")
                else:
                    put_text("无可用的截图方式，请检查 ADB 连接").style("color: var(--bs-warning)")

        put_button(
            label="截图性能测试 (Benchmark)",
            onclick=_benchmark_screenshot,
        )
        put_scope("benchmark_results")
    ...
    def show(self) -> None:
        ...
        with use_scope("content"):
            ...
            put_markdown(
                """
            E7 Shop Bot 是一款免费开源的第七史诗自动脚本。
            E7 Shop Bot is a free open source auto script for Epic Seven.
            Project repository 项目地址：`https://github.com/your-username/e7-shop-bot`
            """
            ).style("text-align: center")
    ...
    def run(self) -> None:
        # setup gui
        set_env(title="E7 Shop Bot", output_animation=False)
...
def clearup():
    ...
    State.clearup()
    task_handler.stop()
    logger.info("E7 closed.")


def app():
    parser = argparse.ArgumentParser(description="E7 web service")
    parser.add_argument(
