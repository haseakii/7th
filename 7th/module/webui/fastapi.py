...
def asgi_app(
    ...
    **starlette_settings
    ...
    # on_startup/on_shutdown 通过 lifespan 实现（Starlette/FastAPI 新版）
    # 此处直接调用，因为 Starlette 在新版中已移除这两个参数
    if on_startup:
        for handler in (on_startup if isinstance(on_startup, (list, tuple)) else [on_startup]):
            handler()
    ...
    if on_shutdown:
        import atexit
        for handler in (on_shutdown if isinstance(on_shutdown, (list, tuple)) else [on_shutdown]):
