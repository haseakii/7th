...
class PurchaseEngine:
    ...
    配置接受方式:
      - buy_list: Dict[str, bool] — 物品类型到是否购买的映射
      - 兼容旧接口：接受 ConfigManager 或 E7Config，自动提取买列表
    """

    def __init__(self, device: DeviceController, config=None, buy_list: Dict[str, bool] = None):
        self.device = device
        self._ocr = OCR
    ...
    def should_buy(self, item: ShopItem) -> bool:
        """根据配置和货币类型判断是否应购买该物品。"""
        if item.item_type in ("unknown", "hero", "currency"):
            return False
